...
TASKS[C3]="Create a Bash script called scripts/monitor.sh that: 1) Monitors CPU and memory usage every 5 seconds, 2) Logs timestamp, CPU%, and memory% to a CSV file (monitor_log.csv), 3) Prints a warning to stderr if CPU > 80% or memory > 90%, 4) Accepts --interval and --output flags for customization, 5) Handles SIGINT gracefully (prints summary before exit). Make it work on both Linux and macOS."
